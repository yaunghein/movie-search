...
const MovieCards = () => {
    ...
    const { movies, toggleFavorite, removeFromFavorite } = useContext(Context);
    ...
    const movieCards = filterMovies.map((movie) => (
        <div className="card-wrapper" key={movie.id}>
            ...
            {movie.isFavorite ? (
                <i className="ri-heart-fill heart" onClick={() => removeFromFavorite(movie)}></i>
            ) : (
                <i className="ri-heart-line heart" onClick={() => toggleFavorite(movie.id)}></i>
            )}
        </div>
    ));
